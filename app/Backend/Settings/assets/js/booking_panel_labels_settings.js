(function ($)
{
	...
	$(document).ready(function ()
	{
		...
		}).on('mouseenter', '[data-step-id]', function ( )
		{
			if( is_in_edit_mode )
				return;
			$(this).find('#translate_edit_icon').show()

		}).on('mouseleave', '[data-step-id]', function ()
		{
			if( is_in_edit_mode )
				return;
			$(this).find('#translate_edit_icon').hide()
		}).on('click', '#translate_edit_icon', function ()
		{
			is_in_edit_mode = true;
			$(this).hide();
			const stepContext = $(this).prev();
			stepContext.addClass('in_edit_mode').attr('contenteditable', true).focus();
			$(this).next().addClass('d-inline-block');
			$(this).next().find('#translate_cancel_icon').data('old-translation', stepContext.text().trim());

			function selectAllText() {
				const container = stepContext[0];
		...
		{
			$(this).parent().removeClass('d-inline-block');
			const oldTranslation = $(this).data('old-translation');
			$(this).parent().prev().prev().text(oldTranslation);
			$('.in_edit_mode').text( oldTranslation ).removeClass('in_edit_mode').removeAttr('contenteditable');
			is_in_edit_mode = false;
		}).on('click', '#translate_save_icon', function ()
		{
			const translationKey = $('.in_edit_mode').data('translate');
			const newTranslation = $('.in_edit_mode').text().trim();

			$('#booknetic_panel_area [data-translate="' + (translationKey.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g,'\\$1')) + '"]').text( newTranslation );
			$('#booknetic_panel_area [data-translate-key="' + (translationKey.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g,'\\$1')) + '"]').val( newTranslation );
